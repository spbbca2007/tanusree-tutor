...
export function defaultState() {
  return { stars:0, streak:0, lastActiveDate:null, hintCount:0, skillHistory:{}, stageVisits:{}, redoHistory:{}, attempts:[], examRecords:[], dailyActivity:{} };
}
...
export function getOverallMastery(s) { const m=curriculum.topics.map(t=>getTopicMastery(s,t.id)); const nz=m.filter(x=>x>0); if(!nz.length) return 0; return Math.round(nz.reduce((a,b)=>a+b,0)/nz.length); }
...
export function getRecommendation(s) { const sc=curriculum.topics.map(t=>({topic:t,mastery:getTopicMastery(s,t.id),attempted:!!(s.skillHistory?.[t.id]&&Object.keys(s.skillHistory[t.id]).length>0)})); const started=sc.filter(x=>x.attempted&&x.mastery<80); if(started.length){started.sort((a,b)=>a.mastery-b.mastery);return{topic:started[0].topic,reason:"Needs practice"};} const un=sc.filter(x=>!x.attempted); if(un.length) return{topic:un[0].topic,reason:"New topic"}; sc.sort((a,b)=>a.mastery-b.mastery); return{topic:sc[0].topic,reason:"Review"}; }
